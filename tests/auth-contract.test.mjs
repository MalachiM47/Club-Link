...
test('officer sign-in uses signInWithPassword with only email trimming', () => {
  assert.match(authSource, /\.auth\.signInWithPassword\(\{\s*email: email\.trim\(\),\s*password,\s*\}\)/s);
...
test('form submits the exact email and password field values', () => {
  assert.match(
    appSource,
    /signInOfficer\(data\.get\('email'\), data\.get\('password'\)\)/,
  );
});

test('authentication errors log only their code and message', () => {
  assert.match(authSource, /code: error\.code \?\? null/);
  assert.match(authSource, /message: error\.message/);
